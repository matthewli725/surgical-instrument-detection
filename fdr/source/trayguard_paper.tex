...
\cite{lind-2026a};
\cite{nichol-et-al-2024};
\cite{ast-surgical-instrumentation-guide};
\cite{encyclopedia-surgical-instruments-2018};
\cite{czarnowski-2025}. For example, a tray requiring four 14~cm Kelly clamps
...
\cite{cdc};
\cite{cdc-healthcare-equipment-accessed-2026};
\cite{cdc-environmental-surfaces-accessed-2026};
\cite{steris-cleaning-instruments-accessed-2026};
\cite{public-health-ontario-2013};
\cite{pennsylvania-patient-safety-authority-2006};
\cite{medline-custom-trays-2025};
\cite{medline-transport-2026}.
...
\cite{hspa-crcst-content-outline-2023};
\cite{hspa-urology-instruments-accessed-2026}.
...
\cite{osha-central-sterile-supply-accessed-2026};
\cite{cdc-niosh-2024};
\cite{osha-computer-workstations-accessed-2026}. Ergonomic improvements compete
...
\cite{brozak-2025};
\cite{onet-medical-equipment-preparers-accessed-2026}.
...
\cite{bls-union-2023};
\cite{trends-in-labor-unionization-2022};
\cite{ashhra-2024}.
...
\cite{kuppers-et-al-2022};
\cite{guo-et-al-2017}, and the operating point would be refined after
...
\cite{wang-et-al-2016-apriltag};
\cite{garrido-jurado-et-al-2014};
\cite{romero-ramirez-et-al-2018}. Compared with
...
\cite{opencv-aruco-detection-accessed-2026}.
...
\cite{issenberg-et-al-2005};
\cite{bell-et-al-2008}.
...
learner rates confidence (1--5 or \texttt{Not sure}). The app records
...
\begin{figure}[H]
\centering
\caption{Functional block diagram showing data flow among subsystems. Rectangles represent the nine functional subsystems from Table~\ref{tab:functional-subsystem-breakdown}; arrows indicate the primary direction of data or control flow.}
\label{fig:functional-block-diagram}
\begin{tikzpicture}[
    node distance=0.65cm and 1.8cm,
    block/.style={rectangle, draw, rounded corners=2pt, minimum width=4cm, minimum height=0.65cm, align=center, font=\small, inner sep=3pt},
    arrow/.style={->, >=stealth, thick},
    returnarrow/.style={->, >=stealth, thick, dashed},
    every path/.style={font=\scriptsize}
]

% Nodes — main pipeline (left column)
\node[block] (content)   {Local content/tray module};
\node[block, below=of content]  (workflow)  {Learning workflow};
\node[block, below=of workflow] (retrieval) {Retrieval practice};
\node[block, below=of retrieval] (scanning)  {AprilTag card scanning};
\node[block, below=of scanning]  (scoring)   {Simulated tray scoring};
\node[block, below=of scoring]   (feedback)  {Feedback engine};
\node[block, below=of feedback]  (reporting) {Reporting/export};
\node[block, below=of reporting] (instructor){Instructor review};

% Nodes — auxiliary column (right)
\node[block, right=of scoring]   (confidence){Confidence/uncertainty\\capture};

% Vertical arrows — main pipeline
\draw[arrow] (content)   to node[right]{instrument templates, tray definitions} (workflow);
\draw[arrow] (workflow)  to node[right]{mode routing, assessment variants}    (retrieval);
\draw[arrow] (retrieval) to node[right]{prompts, card assignments}           (scanning);
\draw[arrow] (scanning)  to node[right]{detected card IDs, timestamps}       (scoring);
\draw[arrow] (scoring)   to node[right]{comparison results, accuracy}        (feedback);
\draw[arrow] (feedback)  to node[right]{error categories, corrective hints}  (reporting);
\draw[arrow] (reporting) to node[right]{evidence packages, summaries}        (instructor);

% Horizontal — confidence capture ↔ scoring
\draw[arrow] (scoring.east)  to node[above]{scored results}         (confidence.west);
\draw[arrow] (confidence.west) to node[below]{confidence ratings, \texttt{Not sure}} (scoring.east);

% Feedback loop — instructor → content
\draw[returnarrow] (instructor.west) -- ++(-1.8,0) node[above, midway, sloped]{content revisions, verification updates} |- (content.west);

% Label
\node[above=0.1cm of content, font=\small\itshape] {Data / control flow};
\end{tikzpicture}
\end{figure}

\begin{figure}[H]
\centering
\caption{Dependency diagram showing cost (red dashed) and value (green solid) inputs/outputs for the nine subsystems. External inputs (left) feed into the system; value flows between blocks (green); cost flows accumulate into right-side summation blocks (red). Constraint icons ($\leq$ / $\geq$) mark the highest-risk dependency wires.}
\label{fig:dependency-diagram}
\begin{tikzpicture}[
    block/.style={rectangle, draw, thick, rounded corners=3pt, minimum width=2.6cm, minimum height=0.7cm, align=center, font=\small, inner sep=2pt},
    external/.style={rectangle, draw, thick, rounded corners=8pt, minimum width=1.5cm, minimum height=0.5cm, align=center, font=\small, fill=gray!15, inner sep=1pt},
    aggregator/.style={rectangle, draw, thick, minimum width=1.0cm, minimum height=0.7cm, align=center, font=\small, inner sep=2pt},
...
% 3×3 grid of subsystems
\matrix[matrix of nodes, column sep=2.2cm, row sep=1.2cm,
    nodes={block, anchor=center}]
{
    |(content)| Content\&module & |(workflow)| Learning\&workflow & |(instructor)| Instructor\&review \\
    |(retrieval)| Retrieval\&practice & |(scanning)| AprilTag\&scanning & |(scoring)| Simulated\&scoring \\
    |(feedback)| Feedback\&engine & |(confidence)| Confidence\&capture & |(reporting)| Reporting\&export \\
};

% External inputs (left side)
\node[external, left=1.5cm of scanning] (camera) {Camera};
\node[external, left=1.5cm of content] (expert) {Expert};
\node[external, left=1.5cm of feedback] (computing) {Computing};

% Aggregators (right side)
\node[aggregator, right=1.5cm of content] (storagesum) {$\Sigma$};
\node[right=0.15cm of storagesum, font=\tiny, align=left] {Storage\&$[$MB$]$};

\node[aggregator, right=1.5cm of instructor] (expertsum) {$\Sigma$};
\node[right=0.15cm of expertsum, font=\tiny, align=left] {Expert time\&$[$hours$]$};

\node[aggregator, right=1.5cm of scanning, yshift=0.7cm] (hardwaresum) {$\Sigma$};
\node[right=0.15cm of hardwaresum, font=\tiny, align=left] {Hardware\&$[$px$]$};

\node[aggregator, right=1.5cm of scanning, yshift=-0.7cm] (compsum) {$\Sigma$};
\node[right=0.15cm of compsum, font=\tiny, align=left] {Computation\&$[$ops/s$]$};

% ── External input arrows ──

% Camera → Scanning
\draw[valuearrow] (camera) -- (scanning) node[midway, above, font=\tiny, sloped] {camera frames};
\draw[costarrow] (camera) -- (scanning) node[midway, below, font=\tiny, sloped] {camera resolution [px]};

% Expert → Content
\draw[valuearrow] (expert) -- (content) node[midway, above, font=\tiny, sloped] {instrument knowledge};
\draw[costarrow] (expert) -- (content) node[midway, below, font=\tiny, sloped] {expert time [h]};

% Expert → Instructor
\draw[valuearrow] (expert) -- (instructor) node[midway, above, font=\tiny, sloped] {review expertise};
\draw[costarrow] (expert) -- (instructor) node[midway, below, font=\tiny, sloped] {expert time [h]};

% Computing → Σ Computation
\draw[costarrow] (computing.east) -- ++(0.5,0) |- (compsum.west) node[pos=0.25, above, font=\tiny] {computation [ops/s]};

% ── Value arrows between blocks ──

\draw[valuearrow] (content) -- (workflow) node[midway, above, font=\tiny] {instrument definitions};
\draw[valuearrow] (content) -- (retrieval) node[midway, left, font=\tiny] {study card content};
\draw[valuearrow] (content.east) -- ++(0.4,0) |- (scoring.west) node[pos=0.25, above, font=\tiny] {tray templates};
\draw[valuearrow] (workflow) -- (retrieval) node[midway, above, font=\tiny] {mode config};
\draw[valuearrow] (retrieval) -- (scanning) node[midway, above, font=\tiny] {card prompts};
\draw[valuearrow] (scanning) -- (scoring) node[midway, above, font=\tiny] {detected card IDs};
\draw[valuearrow] (scoring) -- (feedback) node[midway, right, font=\tiny] {comparison results};
\draw[valuearrow] (scoring) -- (confidence) node[midway, above, font=\tiny] {scored results};
\draw[valuearrow] (confidence) -- (scoring) node[midway, below, font=\tiny] {confidence ratings};
\draw[valuearrow] (feedback) -- (reporting) node[midway, below, font=\tiny] {error summaries};
\draw[valuearrow] (reporting) -- (instructor) node[midway, right, font=\tiny] {evidence packages};
\draw[valuearrow, dashed] (instructor) -- (content) node[midway, above, font=\tiny, sloped] {content revisions};

% ── Cost arrows to aggregators ──

\draw[costarrow] (content) -- (storagesum) node[midway, above, font=\tiny] {storage [MB]};
\draw[costarrow] (content.east) -- ++(0.4,0) |- (expertsum.west) node[pos=0.25, above, font=\tiny] {expert time [h]};
\draw[costarrow] (scanning.east) -- ++(0.4,0) |- (hardwaresum.west) node[pos=0.25, above, font=\tiny] {camera resolution [px]};
\draw[costarrow] (scanning.east) -- ++(0.4,0) |- (compsum.west) node[pos=0.25, below, font=\tiny] {computation [ops/s]};
\draw[costarrow] (scoring.east) -- ++(0.4,0) |- (compsum.west) node[pos=0.35, below, font=\tiny] {computation [ops/s]};
\draw[costarrow] (feedback.east) -- ++(0.4,0) |- (compsum.west) node[pos=0.45, below, font=\tiny] {computation [ops/s]};
\draw[costarrow] (confidence.east) -- ++(0.4,0) |- (compsum.west) node[pos=0.55, below, font=\tiny] {computation [ops/s]};
\draw[costarrow] (reporting) -- (storagesum) node[midway, below, font=\tiny] {storage [MB]};
\draw[costarrow] (instructor) -- (expertsum) node[midway, above, font=\tiny] {expert time [h]};
\draw[costarrow] (workflow.east) -- ++(0.4,0) |- (compsum.west) node[pos=0.65, below, font=\tiny] {computation [ops/s]};
\draw[costarrow] (retrieval.east) -- ++(0.4,0) |- (compsum.west) node[pos=0.75, below, font=\tiny] {computation [ops/s]};

% ── Constraint icons on high-risk wires ──

% Camera → Scanning: resolution ≥ 720 px
\node[constraint] at ($(camera)!0.5!(scanning)$) {$\geq$};

% Expert → Content: time ≤ 8 h
\node[constraint] at ($(expert)!0.5!(content)$) {$\leq$};

% Expert → Instructor: time ≤ 4 h
\node[constraint] at ($(expert)!0.5!(instructor)$) {$\leq$};

% Scanning → Scoring: detection accuracy ≥ 95%
\node[constraint] at ($(scanning)!0.5!(scoring)$) {$\geq$};

% Legend
\node[below=0.3cm of feedback, anchor=north, font=\tiny, align=left]
  {\textcolor{green!50!black}{$\blacksquare$}~Value flow (green) \quad \textcolor{red!60!black}{$\blacksquare$}~Cost flow (red dashed)};
...
Reference + 45\degree & 0.998 & 1.000 & 0.995 & 0.987 \\
Reference + 45\degree + 90\degree & 0.999 & 1.000 & 0.995 & 0.990 \\
\hline
\end{tabularx}
\end{table}
...
\section*{Appendix B: Bills of Materials}
...
\section*{Appendix C: Collaboration Task Matrix}
...
\hline
Appendices (A--C) & & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ \\
\hline
Support & $\checkmark$ & & & & \\
\hline
\end{tabularx}
