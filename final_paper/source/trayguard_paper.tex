...
\cite{huang-et-al-2025}. These findings support
TrayGuard's role as a repeatable training supplement:
local SPD practice varies and depends on
supervised experience with site-specific trays, sheets, and policies.
...
The scale of local variation is substantial. A multi-specialty tray optimization
project at a Danish university hospital considered 1,340 different instrument
trays across all surgical specialties; introducing modular trays reduced
instruments in some existing trays while others required structural changes to
the tray set itself \cite{rubak-et-al-2024}. In
a major academic hospital in the Netherlands, a single general-surgery tray
contained 94 reusable instruments used across 162 procedures in multiple
disciplines, yet only 19 instruments were used per case on average and 10 were
never used \cite{eussen-et-al-2026}. Tray
optimization---removing rarely used instruments---reduced that tray's size and
weight by 54\% \cite{eussen-et-al-2026}. These
figures illustrate why general instrument familiarity is insufficient: a learner
must master not only instrument shapes and names but also the specific tray
configuration, quantities, and local naming conventions of their facility.
...
Mean test scores improved from 41\% at pre-test to 84\% after the workshop, and
remained at 90\% after two months, with surveyed trainees reporting greater
...
\section{Reason for the Training-Centered Pivot \hfill {\normalfont\small (Matthew)}}
...
parts of that pipeline, but does not claim deployment-grade
...
The pivot matches the evidence:
sterile-processing work depends on local knowledge and supervised practice, and
simulation/retrieval evidence supports repeated practice with feedback. It is
buildable with low-cost materials: a file-backed tray module, printable
cards, a scanning surface, scoring, and export. It also reduces risk for a
