# Planning

This folder contains the project execution plan, current task backlog, and
weekly progress snapshots.

## Planning Files

- `future_work/gitlab_epic_map.md`: epic and milestone structure
- `future_work/unit_task_list.md`: task list grouped by milestone
- `future_work/gitlab_issue_table.md`: tabular issue view
- `claim_evidence_control_sheet.md`: claim ownership and citation triage
- `training_feature_prioritization.md`: ranked training feature analysis
- `TODO.md`: consolidated action list
- `weekly_updates/`: weekly progress logs

## Weekly Update Files

- `weekly_updates/week_01.md`
- `weekly_updates/week_02.md`
- `weekly_updates/week_03.md`
- `weekly_updates/week_04.md`
- `weekly_updates/week_05.md`
- `weekly_updates/week_06.md`
- `weekly_updates/week_07.md`
- `weekly_updates/week_08.md`
- `weekly_updates/week_09.md`
- `weekly_updates/week_10.md`
