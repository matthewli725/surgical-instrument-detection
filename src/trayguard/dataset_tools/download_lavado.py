...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="trayguard download-lavado",
        description="Download the Lavado surgical tools dataset from Kaggle and export it as YOLO data.",
    )
    parser.add_argument("--dataset", default=DEFAULT_DATASET, help="Kaggle dataset slug.")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR, help="YOLO dataset output directory.")
    ...
    parser.add_argument("--require-labels", action="store_true", help="Fail when an image has no matching label file.")
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    samples = discover_samples(images_dir=images_dir, labels_dir=labels_dir, require_labels=args.require_labels)
    splits = split_samples(samples=samples, train_ratio=args.train_ratio, val_ratio=args.val_ratio, seed=args.seed)
